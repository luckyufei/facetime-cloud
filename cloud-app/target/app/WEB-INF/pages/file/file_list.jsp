				...
				<div class="row-fluid box">
					<div class="box-content page-list">
							<ul rel="list" id="js_data_list_inner">
                             <c:forEach items="${user.currentFolder.children}" var="subFile">
                               <c:if test="${subFile.type eq 'FOLDER'}">
                               <li rel="item" file_type="${subFile.type}" is_share="${subFile.shared}" file_id="${subFile.folderId}"
                                   file_count="${subFile.fileCount}" folder_count="${subFile.folderCount}" total_size="${subFile.totalSize}"
                                   create_date="<fmt:formatDate  value="${subFile.createDate}" type="date" pattern="yyyy-MM-dd" />" 
                                   modify_date="<fmt:formatDate  value="${subFile.modifyDate}" type="date" pattern="yyyy-MM-dd" />">
                                 <input type="checkbox" value="${subFile.folderId}">
                                 <div class="fs-checkbox"></div>
                                 <i class="file-type tp-folder"></i>
                                 <div class="file-name">
                                  <a href="javascript:;" cate="1" btn="goto_dir" aid="1" cid="2220068" rel="view_folder">${subFile.folderName}</a>
                                 </div>
                                 <div class="file-info">
                                  <em><fmt:formatDate  value="${subFile.modifyDate}" type="date" pattern="yyyy-MM-dd" /></em> 
                                  <span class="file-ctrl" style="display: block;">
